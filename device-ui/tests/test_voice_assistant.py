...
def test_voice_assistant_default_wake_phrase_is_nexa(monkeypatch):
    monkeypatch.delenv("VOICE_ASSISTANT_WAKE_PHRASE", raising=False)
    assistant = VoiceAssistant(lambda _intent: None)
    assert assistant.wake_phrase == "hey nexa"


def test_combined_wake_and_command_starts_meeting():
    interpreter = _mk()
    intent = interpreter.handle_transcript("hey nexa start meeting", now=10.0)
...
def test_fuzzy_wake_phrase_allows_minor_transcript_variation():
    interpreter = _mk()
    assert interpreter.handle_transcript("hey nexus", now=10.0) is None
...
def test_playback_like_false_wake_does_not_open_pause_command_window():
    interpreter = _mk()
    assert interpreter.handle_transcript("the next item is ready", now=10.0) is None
...
def test_brightness_command_captures_value():
    interpreter = _mk()
    intent = interpreter.handle_transcript("hey nexa brightness medium", now=10.0)
    assert intent is not None
...
def test_unsupported_command_returns_fallback_intent():
    interpreter = _mk()
    intent = interpreter.handle_transcript("hey nexa speaker test", now=10.0)
    assert intent is not None
...
def test_help_command_is_supported():
    interpreter = _mk()
    intent = interpreter.handle_transcript("hey nexa what can you do", now=10.0)
    assert intent is not None
...
def test_wake_only_utterance_detected():
    interpreter = _mk()
    assert interpreter.is_wake_only_utterance("hey nexa") is True
    assert interpreter.is_wake_only_utterance("please hey nexa uh") is True


def test_non_wake_only_with_extra_words():
    interpreter = _mk()
    assert interpreter.is_wake_only_utterance("hey nexa what time is it") is False
...
def test_farewell_see_you_later_short_utterance():
    assert utterance_is_voice_farewell("hey nexa", "see you later") is True
    assert utterance_is_voice_farewell("hey nexa", "ok see you later") is True
