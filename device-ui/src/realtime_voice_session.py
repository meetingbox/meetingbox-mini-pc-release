...
class RealtimeVoiceSession:
    ...
    async def _cancel_for_local_barge_in(
        ...
        *,
        ...
        # Restore eager response.cancel (previously removed in aa7c21c to
        # avoid permanent-silence on false positives). The physical raw-ratio
        # gate added in 2bf6941 (raw_mic_rms >= ref_rms * 2.0) now makes it
        # physically impossible for echo alone to trigger this path - a room
        # can only attenuate the speaker, never amplify - so the false-
        # positive concern that motivated aa7c21c no longer applies to
        # aec_verified. Without this cancel, OpenAI kept its response state
        # active, mic upload stayed gated under half-duplex, and the session
        # got stuck showing 'listening' while OpenAI never advanced to the
        # user's new turn.
        try:
            await ws.send(json.dumps({"type": "response.cancel"}))
