...
At kiosk login, **`meetingbox-gdm-kiosk-session`** runs **`/usr/local/bin/meetingbox-apply-kiosk-display-orientation`**, which reads **`/etc/meetingbox/panel-xrandr.env`** (installed once by **`install-gdm-kiosk-session.sh`** from **`kiosk-desktop/panel-xrandr.env.example`**). Defaults: **`DSI-1`**, **`800x1280`**, **`rotate left`**. Edit that file to match your **`xrandr`** output, then reboot. Set **`MEETINGBOX_SKIP_PANEL_XRANDR=1`** there to disable. For Docker UI scaling, set **`MEETINGBOX_SYNC_DISPLAY_FROM_XRANDR=1`** or **`DISPLAY_WIDTH` / `DISPLAY_HEIGHT`** in **`.env`** to match **`xrandr`** after rotation.

**Touch misaligned or not working after rotation:** the same script runs **`xinput map-to-output`** so the touchscreen is tied to the same output as the panel. **`xinput`** is installed with **`install-gdm-kiosk-session.sh`**. Set **`MEETINGBOX_TOUCH_XINPUT_DEVICE`** to the exact line from **`xinput list --name-only`** if needed. If the **UI looks right but taps hit the wrong spot**, for **`--rotate left`** use **`MEETINGBOX_TOUCH_MATRIX_PRESET=left`** (in the example file by default); if taps are ~90° off (wrong edge), try **`MEETINGBOX_TOUCH_COORD_MATRIX="0 1 0 1 0 0 0 0 1"`** (swap X/Y) instead. For **`--rotate right`** use **`MEETINGBOX_TOUCH_MATRIX_PRESET=right`** — see **`kiosk-desktop/panel-xrandr.env.example`**. Also set **`MEETINGBOX_SYNC_DISPLAY_FROM_XRANDR=1`** or **`DISPLAY_WIDTH` / `DISPLAY_HEIGHT`** in **mini-pc `.env`** to match **`xrandr`** after rotation (wrong Kivy size skews hit targets). Add the GUI user to **`input`**: **`sudo usermod -aG input meetingbox`**. **`sudo evtest`** checks the kernel.
